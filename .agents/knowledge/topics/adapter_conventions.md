...
All 17 adapters resolve correctly via default ndim inference — none override `LATENT_AXES`:
...
| MiniMax H3 T2VA/FL2VA/Ref2VA | Per component: video VIDEO, audio PACKED |
...
10. **Latent geometry override is rarely needed** — `resolve_latent_axes` infers axis roles from latent ndim (3=packed, 4=conv, 5=video), correct for all 17 adapters. Set the `LATENT_AXES` ClassVar only for a genuinely non-standard rank/channel layout. LTX2's packed `[video|audio]` resolves as PACKED; its modality split lives in the adapter `forward` (`video_seq_len`), not the geometry layer. See "Latent Geometry".
...
13. **Runtime and trajectory ownership stay explicit** — component declarations are not materialized lifecycle roots, prepared/replacement overrides are not canonical backend ownership, and component mappings never define scheduler/RNG order. Multi-component adapters publish authoritative `StructuredTrajectory` data and expose it through adapter bridge APIs; do not mirror it into legacy fields. See [Component Runtime](component_runtime.md) and [Structured Trajectory](structured_trajectory.md).
14. **Lazy materialization preserves global lifecycle policy** — a component loaded after adapter construction must still receive `frozen_parameters_dtype`, and FSDP CPU-efficient loading must synchronize module parameters and buffers before first use in preprocessing or inference. Non-module declarations are skipped. Load stage components through `on_load_components()`; do not call a lazy pipeline loader directly from model code.
15. **`**kwargs` preprocessors declare cache semantics** — when `preprocess_func` hides semantic inputs behind `**kwargs`, list every output-affecting field in `preprocess_cache_fields` and bump `preprocess_cache_version` when helper behavior changes. Dataset source bytes are hashed automatically; media replaced in place at the same path still requires `force_reprocess: true`.
