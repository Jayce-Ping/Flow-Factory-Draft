...
8. **Bagel batch handling (NaViT packing vs per-sample fallback)** — Bagel uses sequence packing, not a leading batch dim, so it dispatches by task: **T2I** packs all B prompts into one block-diagonal forward (`_build_gen_context_batched` + `_forward_packed`; the framework's `(B, num_tokens, dim)` latents reshape to packed `(B*num_tokens, dim)` and back), while **I2I** warns once and falls back to per-sample (`batch_size=1`) in both `inference()` and `forward()` because reference-image sizes vary per sample. The dispatch flag `_is_i2i(condition_images)` depends only on condition-image presence, so it is identical across ranks (distributed-safe; mirrors `flux2_klein.py` "Approach 2"). CFG global renorm is computed **per sample** over `packed_seqlens - 2`, and `forward()` returns per-sample `(B,)` log-prob (not per-token) to honor the trainer ratio contract.
9. **Image columns persist via HF Image feature (variable-size/count I2I)** — preprocessing stores image data as PIL via the HF `Image` feature, not raw tensors; ragged tensor columns (multi-reference images of varying size/count) are NOT Arrow-serializable and otherwise crash in `Dataset.map` with `TypeError: a bytes-like object is required, not 'Tensor'` / `OverflowError`. The raw `images` column is always stored this way; an `encode_image` output is stored this way only when its name is listed in the adapter's `preprocess_image_columns` ClassVar (default empty — opt in for RGB images only, e.g. Bagel `condition_images`). These columns **read back as PIL** (`List[List[PIL.Image]]`); the `torch` format excludes them (`_apply_torch_format` in `dataset.py`), and `collate_fn` keeps them as a `MultiImageBatch`. Bump `_PREPROCESS_FORMAT_VERSION` if the on-disk image format changes again.
