...
The `AdvantageProcessor` is instantiated in `BaseTrainer._init_reward_model()` with `sampler_type=self.config.data_args.sampler_type`. Reward-based trainers delegate advantage computation to `self.advantage_processor.compute_advantages()` via their own `compute_advantages()` method, invoked from `prepare_feedback()` after each online rollout iteration (see `guidance/workflow.md` for `sample` → `prepare_feedback` → `optimize`). Reward-free online distillation trainers declare `ONLINE_NO_FEEDBACK_EXECUTION_CONTRACT`, so the shared feedback gate skips `prepare_feedback()` and `AdvantageProcessor` dispatch. Online DPO forms chosen/rejected pairs at the start of `optimize()`, not in `prepare_feedback()`. DGPO handles group loss in its own `_compute_group_dgpo_loss()` via `scatter_add + reduce`.
