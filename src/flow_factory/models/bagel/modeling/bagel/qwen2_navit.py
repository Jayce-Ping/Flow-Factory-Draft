...
class PackedAttentionMoT(Qwen2Attention):
    ...
    def forward_inference(
        ...
        if mode == "und":
            ...
            packed_query_sequence_[packed_text_indexes] = self.o_proj(
                packed_attn_output[packed_text_indexes]
            )
            packed_query_sequence_[packed_vae_token_indexes] = self.o_proj_moe_gen(
    ...
    def forward_inference(
        ...
        if enable_taylorseer:
            if self.current["type"] == "full":
                derivative_approximation(
                    cache_dic=self.cache_dic, current=self.current, feature=packed_query_sequence
                )
