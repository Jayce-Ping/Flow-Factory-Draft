...
def posterior_mode(encoder_output: Any, *, source: str) -> torch.Tensor:
    """Resolve the deterministic posterior mode used by official pipelines.
    ...
    Returns:
        Deterministic posterior-mode latent tensor.

    Raises:
        TypeError: If the encoder surface cannot yield one tensor mode.
    """
    ...
    if latent_dist is None and isinstance(encoder_output, (tuple, list)):
        ...
        latent_dist = getattr(encoder_output[0], "latent_dist", encoder_output[0])
    if latent_dist is None and getattr(encoder_output, "mode", None) is not None:
        latent_dist = encoder_output
    mode = getattr(latent_dist, "mode", None)
    latents = mode() if callable(mode) else mode
    if not isinstance(latents, torch.Tensor):
        raise TypeError(
            f"{source} expected VAE posterior mode to be torch.Tensor, "
...
def encode_shift_scale_vae_image(
    ...
    *,
    ...
    """Apply a deterministic KL posterior and Diffusers shift/scale transform.
    ...
    Returns:
        Deterministic shift/scale-normalized VAE latents.
    """
    source = source or f"{type(adapter).__name__} VAE encode"
    vae = adapter.vae
    latents = posterior_mode(vae.encode(pixel_values), source=source)
