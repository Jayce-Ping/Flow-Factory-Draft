...
def encode_flux1_vae_image(adapter: Any, pixel_values: torch.Tensor) -> torch.Tensor:
    """Apply the deterministic VAE transform shared by FLUX.1 roles.
    ...
    """
    return encode_shift_scale_vae_image(adapter, pixel_values)
...
def encode_flux2_output_images(
    ...
    """Apply official FLUX.2 VAE mode, patchify, BatchNorm, and token packing.

    Args:
        adapter: FLUX.2 adapter exposing pipeline packing primitives.
        pixel_values: Preprocessed BCHW target images.
        generator: Accepted for the output-codec boundary; deterministic argmax consumes no RNG.

    Returns:
        Packed clean latents with their position identifiers.
    """
    del generator  # The official FLUX.2 target transform uses posterior argmax.
    latents = encode_flux2_vae_image(adapter, pixel_values)
...
def encode_flux2_vae_image(adapter: Any, pixel_values: torch.Tensor) -> torch.Tensor:
...
def prepare_flux2_condition_latents(
    ...
    *,
    ...
    image_latents = [
        encode_flux2_vae_image(adapter, image.to(device=device, dtype=dtype)) for image in images
    ]
