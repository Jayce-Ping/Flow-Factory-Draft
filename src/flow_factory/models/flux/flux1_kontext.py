...
class Flux1KontextAdapter(ConfiguredImageOutputAdapterMixin, BaseAdapter):
    ...
    def encode_prompt(
        self, prompt: Union[str, List[str]], max_sequence_length: int = 512, **kwargs
    ) -> Dict[str, Any]:
    ...
    def encode_image(
        ...
        if not is_image_batch(images):
            raise ValueError(
                f"Invalid image input type: {type(images)}. Must be a PIL Image, numpy array, torch tensor, or a list of these types."
            )
        ...
        if condition_image_size is None:
            ...
            image_height, image_width = self.pipeline.image_processor.get_default_height_width(
                first_image
            )
        ...
        image_latents = encode_flux1_vae_image(self, image_tensors)
        ...
        return {
            "condition_images": [
                img.to(device)
                for img in self.pipeline.image_processor.postprocess(
                    image_tensors, output_type="pt"
                )
            ],  # convert numerical range to [0, 1]
            "image_latents": image_latents,
            "image_ids": image_ids.unsqueeze(0).expand(
                batch_size, *[-1] * (image_ids.ndim)
            ),  # Expand to batch size
        }

    def encode_video(self, videos: Any) -> None:
        """Return no video condition encoding because Kontext accepts images only."""
        return None
    ...
    def _encode_output_images(
        ...
        """Encode target images and join target/condition position identifiers."""
        del generator
        ...
        target_ids = self.pipeline._prepare_latent_image_ids(
            batch_size,
            latent_height // 2,
            latent_width // 2,
            self.device,
        ...
        image_ids = self._shared_condition_image_ids(condition, batch_size, packed.dtype)
        latent_ids = torch.cat([target_ids, image_ids], dim=0)
    ...
    def prepare_latents(
        ...
        latent_ids = self.pipeline._prepare_latent_image_ids(
