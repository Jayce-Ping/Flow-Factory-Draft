...
"""Target-image encoding shared by Qwen-Image pipeline variants."""
...
from ..configured_image_output import EncodedImageTensor, posterior_mode


def encode_qwen_vae_image(adapter: Any, video_values: torch.Tensor) -> torch.Tensor:
    """Apply the deterministic 5D VAE transform shared by Qwen image roles.
...
def encode_qwen_output_images(
    ...
    *,
    ...
    latents = encode_qwen_vae_image(adapter, video_values)
    latent_channels = latents.shape[1]
