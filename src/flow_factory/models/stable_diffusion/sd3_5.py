...
class SD3_5Adapter(ConfiguredImageOutputAdapterMixin, BaseAdapter):
    ...
    def _encode_output_images(
        ...
        """Encode SD3.5 targets with the role-neutral shift/scale VAE primitive."""
    ...
    def _validate_output_image_context(
        ...
        """Keep SD3.5 decode context limited to its exact H/W inverse."""
        del media_batch, condition
        if set(encoded.decode_context) != {"height", "width"}:
            raise ValueError(
                "SD3.5 decode_context must exactly contain height and width, "
