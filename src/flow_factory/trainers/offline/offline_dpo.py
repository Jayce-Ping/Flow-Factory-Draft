...
"""Finite-dataset diffusion DPO with on-the-fly target encoding."""
...
from ...contracts.execution import OFFLINE_EXECUTION_CONTRACT
...
class OfflineDPOTrainer(BaseTrainer):
    ...
    def _build_train_dataloader(self) -> Tuple[DataLoader, Dict[str, DataLoader]]:
        """Build the already-sharded preference loader without Accelerator wrapping."""
    ...
    def optimize_batch(self, batch: Any) -> None:
        """Apply one gradient-accumulation microstep from an offline preference batch."""
        ...
        chosen_condition = bind_output_forward_context(condition, chosen.forward_context)
        rejected_condition = bind_output_forward_context(condition, rejected.forward_context)
        ...
        with self.accumulate_gradients():
            for timestep in all_timesteps:
                ...
                # Install a full-parameter reference only once per timestep. A
                # separate scope per arm would copy the entire policy to and from
                # the reference snapshot twice without changing the objective.
