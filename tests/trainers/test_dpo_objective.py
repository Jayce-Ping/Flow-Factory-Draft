...
def _legacy_online_objective(
    ...
    """Reproduce the pre-extraction online DPO implementation verbatim."""
...
def test_objective_matches_online_values_and_logging_metrics_exactly() -> None:
    ...
    assert actual_metrics["implicit_reward_chosen"].shape == (3,)
    assert actual_metrics["implicit_reward_rejected"].shape == (3,)
...
def test_online_trainer_delegates_without_renaming_or_reducing_metrics(
    ...
    reference_chosen = torch.tensor([5.0, 6.0])
    reference_rejected = torch.tensor([7.0, 8.0])
    ...
    assert received == {
        "policy_chosen_loss": policy_chosen,
        "policy_rejected_loss": policy_rejected,
        "reference_chosen_loss": reference_chosen,
        "reference_rejected_loss": reference_rejected,
        "beta": 3.5,
    }
